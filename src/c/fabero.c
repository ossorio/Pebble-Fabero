...
static void my_layer_draw(Layer *layer, GContext *ctx) {
  ...
  graphics_context_set_fill_color(ctx, GColorFromHEX(0xcc0000));
...
static void main_window_load(Window *window) {
  ...
  text_layer_set_text_color(s_time_layer_hours, color_red);
  ...
  text_layer_set_text_color(s_time_layer_minutes, GColorBlack);
  ...
  text_layer_set_text_color(s_battery_layer, color_red);
  ...
  layer_add_child(window_layer, text_layer_get_layer(s_weather_layer));
    ...
    s_bitmap = gbitmap_create_with_resource(RESOURCE_ID_ESCUDO_FABERO);
    GSize image_size = gbitmap_get_bounds(s_bitmap).size;
    char image_offsetX = (bounds.size.w - image_size.w) / 2; 
    GRect image_frame = GRect(image_offsetX, text_size.h + y_offset_time_layer + 21, image_size.w, image_size.h);
    ...
    s_bitmap_bw = gbitmap_create_with_resource(RESOURCE_ID_ESCUDO_FABERO);
